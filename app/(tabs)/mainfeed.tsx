      ...
      <View className="px-5 mb-6 flex-row items-center space-x-3">
        <View className="flex-1">
          ...
          {isSearchFocused && filteredSuggestions.length > 0 && (
            <View className={`absolute top-14 left-0 right-0 z-50 ${styles.cardBg} rounded-2xl shadow-lg`}>
              {filteredSuggestions.map((suggestion) => (
                <TouchableOpacity
                  key={suggestion.id}
                  className={`flex-row items-center p-4 border-b ${styles.borderColor}`}
                  onPress={() => {
                    setSearchQuery(suggestion.name);
                    setIsSearchFocused(false);
                  }}
                >
                  <Ionicons name={suggestion.icon} size={20} color={styles.iconColor} />
                  <View className="ml-3">
                    <Text className={styles.textPrimary}>{suggestion.name}</Text>
                    <Text className={styles.textSecondary}>{suggestion.country}</Text>
                  </View>
                  <View className="ml-auto">
                    <Text className={styles.textSecondary}>{suggestion.type}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
        ...
        <TouchableOpacity
          ...
          className={`${styles.bgSecondary} p-3 rounded-full`}
        >
      ...
      >
        <TouchableWithoutFeedback onPress={() => setShowMoreOptions(false)}>
          <View className={`flex-1 ${styles.overlayBg}`}>
            ...
            >
              <View className={`${styles.cardBg} rounded-2xl shadow-xl overflow-hidden`}>
                <TouchableOpacity
                  className={`p-4 flex-row items-center ${styles.bgPrimary} border-b ${styles.borderColor}`}
                  onPress={() => {
                    router.push("/stack/trips");
                    setShowMoreOptions(false);
                  }}
                >
                  <Ionicons name="map-outline" size={20} color={styles.iconColor} />
                  <Text className={`${styles.textPrimary} ml-3`}>View Trip</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  className={`p-4 flex-row items-center ${styles.bgPrimary} border-b ${styles.borderColor}`}
                  onPress={() => {
                ...
                >
                  <Ionicons name="share-social-outline" size={20} color={styles.iconColor} />
                  <Text className={`${styles.textPrimary} ml-3`}>Share Trip</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  className={`p-4 flex-row items-center ${styles.bgPrimary}`}
                  onPress={() => {
                    // Handle view user profile
                    setShowMoreOptions(false);
                  }}
                >
                  <Ionicons name="person-outline" size={20} color={styles.iconColor} />
                  <Text className={`${styles.textPrimary} ml-3`}>View User</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
